...
process_hipass_1hz(register double val)
{
	...
	register double tmp, fir, iir;
	tmp = buf[0];
	memmove(buf, buf + 1, 1 * sizeof(double));
	iir = val * 0.9993212972714023;
	iir -= 0.9986429017039066 * tmp;
	fir = tmp;
	iir -= -1.998642287381703 * buf[0];
	fir += -buf[0] - buf[0];
	fir += iir;
	buf[1] = iir;
	val = fir;
