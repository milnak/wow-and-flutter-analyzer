...
BOOL CWfguiDlg::OnInitDialog()
{
	...
	m_filter_combo.AddString(L"Unweighted");
	m_filter_combo.AddString(L"DIN");
...
void CWfguiDlg::OpenDevice()
{
	...
	{
		StoreError(mRes, FALSE, L"File: %s ,Line Number:%d", __FILE__, __LINE__);
		throw m_csErrorText;
	}
}
...
CString CWfguiDlg::StoreError(MMRESULT mRes, BOOL bDisplay, LPCTSTR lpszFormat, ...)
{
	...
	if (m_bRun)
	{
		...
		if (mRes1 != 0)
			wsprintf(szErrorText, L"Error %d in querying the error string for error code %d", mRes1, mRes);
		m_csErrorText.Format(L"%s: %s", szT1, szErrorText);
		if (bDisplay)
...
#define OLD_DYNAMICS 0
		...
		if (++current_buffer == 10)
		{
			...
			m_rms.Format(L"%.4f", max_RMS[index_100]);
